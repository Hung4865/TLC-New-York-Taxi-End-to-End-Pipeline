...
def create_final_dataframe(df, spark_session):
    ...
    with open('./src/spark_stream/schema_config.json', 'r') as f:
        config = json.load(f)
...
def start_streaming(df):
    ...
    stream_query = df.writeStream \
                        .format("parquet") \
                        .outputMode("append") \
                        .option("path", f"hdfs://{HDFS_HOST}:{HDFS_PORT}/{BUCKET_NAME}/stream/") \
                        .option("checkpointLocation", f"hdfs://{HDFS_HOST}:{HDFS_PORT}/{BUCKET_NAME}/stream/checkpoint") \
                        .start() 
